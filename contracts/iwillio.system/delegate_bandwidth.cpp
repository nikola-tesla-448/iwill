...
#include <iwillio.token/iwillio.token.hpp>


#include <cmath>
#include <map>

namespace iwilliosystem {
   using iwillio::asset;
   using iwillio::indexed_by;
   using iwillio::const_mem_fun;
   using iwillio::bytes;
   using iwillio::print;
   using iwillio::permission_level;
   using std::map;
   using std::pair;
   ...
   struct user_resources {
      ...
      IWILLLIB_SERIALIZE( user_resources, (owner)(net_weight)(cpu_weight)(ram_bytes) )
   };
   ...
   struct delegated_bandwidth {
      ...
      // explicit serialization macro is not necessary, used here only to improve compilation time
      IWILLLIB_SERIALIZE( delegated_bandwidth, (from)(to)(net_weight)(cpu_weight) )
   ...
   struct refund_request {
      account_name  owner;
      time          request_time;
      iwillio::asset  net_amount;
      iwillio::asset  cpu_amount;

      uint64_t  primary_key()const { return owner; }

      // explicit serialization macro is not necessary, used here only to improve compilation time
      IWILLLIB_SERIALIZE( refund_request, (owner)(request_time)(net_amount)(cpu_amount) )
   };
    ...
    */
   typedef iwillio::multi_index< N(userres), user_resources>      user_resources_table;
   typedef iwillio::multi_index< N(delband), delegated_bandwidth> del_bandwidth_table;
   typedef iwillio::multi_index< N(refunds), refund_request>      refunds_table;
   ...
   void system_contract::buyrambytes( account_name payer, account_name receiver, uint32_t bytes ) {
      ...
      auto tmp = *itr;
      auto iwillout = tmp.convert( asset(bytes,S(0,RAM)), CORE_SYMBOL );

      buyram( payer, receiver, iwillout );
   }
   ...
   {
      require_auth( payer );
      iwillio_assert( quant.amount > 0, "must purchase a positive amount" );
      ...
      INLINE_ACTION_SENDER(iwillio::token, transfer)( N(iwillio.token), {payer,N(active)},
         { payer, N(iwillio.ram), quant_after_fee, std::string("buy ram") } );

      if( fee.amount > 0 ) {
         INLINE_ACTION_SENDER(iwillio::token, transfer)( N(iwillio.token), {payer,N(active)},
                                                       { payer, N(iwillio.ramfee), fee, std::string("ram fee") } );
      }
      ...
      iwillio_assert( bytes_out > 0, "must reserve a positive amount" );
   ...
   void system_contract::sellram( account_name account, int64_t bytes ) {
      ...
      iwillio_assert( bytes > 0, "cannot sell negative byte" );

      user_resources_table  userres( _self, account );
      auto res_itr = userres.find( account );
      iwillio_assert( res_itr != userres.end(), "no resource row" );
      iwillio_assert( res_itr->ram_bytes >= bytes, "insufficient quota" );
      ...
      iwillio_assert( tokens_out.amount > 1, "token amount received from selling ram is too low" );
      ...
      iwillio_assert( _gstate.total_ram_stake >= 0, "error, attempt to unstake more tokens than previously staked" );
      ...
      INLINE_ACTION_SENDER(iwillio::token, transfer)( N(iwillio.token), {N(iwillio.ram),N(active)},
                                                       { N(iwillio.ram), account, asset(tokens_out), std::string("sell ram") } );

      auto fee = ( tokens_out.amount + 199 ) / 200; /// .5% fee (round up)
      // since tokens_out.amount was asserted to be at least 2 earlier, fee.amount < tokens_out.amount
      
      if( fee > 0 ) {
         INLINE_ACTION_SENDER(iwillio::token, transfer)( N(iwillio.token), {account,N(active)},
            { account, N(iwillio.ramfee), asset(fee), std::string("sell ram fee") } );
      }
   }
   ...
   void validate_b1_vesting( int64_t stake ) {
      ...
      iwillio_assert( max_claimable - claimable <= stake, "b1 can only claim their tokens over 10 years" );
   }
   ...
   {
      require_auth( from );
      iwillio_assert( stake_net_delta != asset(0) || stake_cpu_delta != asset(0), "should stake non-zero amount" );
      iwillio_assert( std::abs( (stake_net_delta + stake_cpu_delta).amount )
      ...
      // update stake delegated from "from" to "receiver"
      {
         ...
         iwillio_assert( asset(0) <= itr->net_weight, "insufficient staked net bandwidth" );
         iwillio_assert( asset(0) <= itr->cpu_weight, "insufficient staked cpu bandwidth" );
      ...
      // update totals of "receiver"
      {
         ...
         iwillio_assert( asset(0) <= tot_itr->net_weight, "insufficient staked total net bandwidth" );
         iwillio_assert( asset(0) <= tot_itr->cpu_weight, "insufficient staked total cpu bandwidth" );
      ...
      if ( N(iwillio.stake) != source_stake_from ) { //for iwillio both transfer and refund make no sense
         ...
         if( is_delegating_to_self || is_undelegating ) {
            if ( req != refunds_tbl.end() ) { //need to update refund
               ...
               iwillio_assert( asset(0) <= req->net_amount, "negative net refund amount" ); //should never happen
               iwillio_assert( asset(0) <= req->cpu_amount, "negative cpu refund amount" ); //should never happen
         ...
         if ( asset(0) < transfer_amount ) {
            INLINE_ACTION_SENDER(iwillio::token, transfer)( N(iwillio.token), {source_stake_from, N(active)},
               { source_stake_from, N(iwillio.stake), asset(transfer_amount), std::string("stake bandwidth") } );
         }
      }
      ...
      // update voting power
      {
         ...
         iwillio_assert( 0 <= from_voter->staked, "stake for voting cannot be negative");
         if( from == N(b1) ) {
   ...
   {
      iwillio_assert( stake_cpu_quantity >= asset(0), "must stake a positive amount" );
      iwillio_assert( stake_net_quantity >= asset(0), "must stake a positive amount" );
      iwillio_assert( stake_net_quantity + stake_cpu_quantity > asset(0), "must stake a positive amount" );
      iwillio_assert( !transfer || from != receiver, "cannot use transfer flag if delegating to self" );
   ...
   {
      iwillio_assert( asset() <= unstake_cpu_quantity, "must unstake a positive amount" );
      iwillio_assert( asset() <= unstake_net_quantity, "must unstake a positive amount" );
      iwillio_assert( asset() < unstake_cpu_quantity + unstake_net_quantity, "must unstake a positive amount" );
      iwillio_assert( _gstate.total_activated_stake >= min_activated_stake,
   ...
   void system_contract::refund( const account_name owner ) {
      ...
      iwillio_assert( req != refunds_tbl.end(), "refund request not found" );
      iwillio_assert( req->request_time + refund_delay <= now(), "refund is not available yet" );
      // Until now() becomes NOW, the fact that now() is the timestamp of the previous block could in theory
      // allow people to get their tokens earlier than the 3 day delay if the unstake happened immediately after many
      // consecutive missed blocks.

      INLINE_ACTION_SENDER(iwillio::token, transfer)( N(iwillio.token), {N(iwillio.stake),N(active)},
                                                    { N(iwillio.stake), req->owner, req->net_amount + req->cpu_amount, std::string("unstake") } );
