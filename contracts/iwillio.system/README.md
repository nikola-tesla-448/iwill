...
## iwillio.system::cfgproducer    account config
...
## iwillio.system::okproducer     account producer vote
...
## iwillio.system::setproxy       account proxy
...
## iwillio.system::unstake        account quantity 
...
## iwillio.system::withdraw account
   - this action can only be triggered by iwillio.system via a deferred transaction generated by unstake
   - this will generate an inline iwillio.token::transfer call from=iwillio.system to=account and amount equal to the next withdraw increment
   - this will generate another deferred withdraw to continue the process if there are still withdraws to be made


## iwillio.system::transfer from to amount memo
   - decrements balance of from if amount <= balance
   - increments balance of to by amount
   - memo is ignored 

## iwillio.system::stakevote account amount
   - the primary currency of iwillio is controlled by the token contract which enables users to transfer
...
## iwillio.system::onblock   account blocktime blocknum
...
## iwillio.system::freeze producer accounts true|false 
...
## iwillio.system::paystandby producer
