#include <iwillio.msig/iwillio.msig.hpp>
#include <iwilliolib/action.hpp>
#include <iwilliolib/permission.hpp>
...
void multisig::propose() {
   ...
   iwillio_assert( trx_header.expiration >= iwillio::time_point_sec(now()), "transaction expired" );
   //iwillio_assert( trx_header.actions.size() > 0, "transaction must have at least one action" );

   proposals proptable( _self, proposer );
   iwillio_assert( proptable.find( proposal_name ) == proptable.end(), "proposal with the same name exists" );
   ...
   iwillio_assert( res > 0, "transaction authorization failed" );
...
void multisig::approve( account_name proposer, name proposal_name, permission_level level ) {
   ...
   iwillio_assert( itr != apps.requested_approvals.end(), "approval is not on the list of requested approvals" );
...
void multisig::unapprove( account_name proposer, name proposal_name, permission_level level ) {
   ...
   iwillio_assert( itr != apps.provided_approvals.end(), "no approval previously granted" );
...
void multisig::cancel( account_name proposer, name proposal_name, account_name canceler ) {
   ...
   if( canceler != proposer ) {
      iwillio_assert( unpack<transaction_header>( prop.packed_transaction ).expiration < iwillio::time_point_sec(now()), "cannot cancel until expiration" );
   }
...
IWILLIO_ABI( iwillio::multisig, (propose)(approve)(unapprove)(cancel)(exec) )
