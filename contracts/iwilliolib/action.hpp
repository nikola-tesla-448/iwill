...
namespace iwillio {
    ...
    *    int  c; //4
    *
    *    IWILLLIB_SERIALIZE( dummy_action, (a)(b)(c) )
    *  };
   ...
   struct permission_level {
      ...
      IWILLLIB_SERIALIZE( permission_level, (actor)(permission) )
   };
   ...
   struct action {
      ...
      IWILLLIB_SERIALIZE( action, (account)(name)(authorization)(data) )
      ...
      void send_context_free() const {
         iwillio_assert( authorization.size() == 0, "context free actions cannot have authorizations");
         auto serialize = pack(*this);
      ...
      template<typename T>
      T data_as() {
         iwillio_assert( name == T::get_name(), "Invalid name" );
         iwillio_assert( account == T::get_account(), "Invalid account" );
...
::iwillio::inline_dispatcher<decltype(&CONTRACT_CLASS::FUNCTION_NAME), ACTION_NAME>::call

#define INLINE_ACTION_SENDER2( CONTRACT_CLASS, NAME )\
INLINE_ACTION_SENDER3( CONTRACT_CLASS, NAME, ::iwillio::string_to_name(#NAME) )
 ...
 * @param NAME - The name of the action
 */
#define ACTION( CODE, NAME ) struct NAME : ::iwillio::action_meta<CODE, ::iwillio::string_to_name(#NAME) >
