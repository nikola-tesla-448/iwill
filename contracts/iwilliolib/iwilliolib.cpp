...
namespace iwillio {
   ...
   void set_blockchain_parameters(const iwillio::blockchain_parameters& params) {
      char buf[sizeof(iwillio::blockchain_parameters)];
      iwillio::datastream<char *> ds( buf, sizeof(buf) );
      ds << params;
      set_blockchain_parameters_packed( buf, ds.tellp() );
   }

   void get_blockchain_parameters(iwillio::blockchain_parameters& params) {
      char buf[sizeof(iwillio::blockchain_parameters)];
      size_t size = get_blockchain_parameters_packed( buf, sizeof(buf) );
      iwillio_assert( size <= sizeof(buf), "buffer is too small" );
      iwillio::datastream<const char*> ds( buf, size_t(size) );
      ds >> params;
   }
   ...
   private:
      ...
      public:
         ...
         {
            iwillio_assert(_offset == _heap_size, "malloc_from_freed was designed to only be called after _heap was completely allocated");
...
{
   return iwillio::memory_heap.malloc(size);
}

void* calloc(size_t count, size_t size)
{
   void* ptr = iwillio::memory_heap.malloc(count*size);
...
{
   return iwillio::memory_heap.realloc(ptr, size);
}

void free(void* ptr)
{
   return iwillio::memory_heap.free(ptr);
}
