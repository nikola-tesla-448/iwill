...
class simpletoken : public iwillio::contract {
   public:
      ...
      void transfer( account_name from, account_name to, uint64_t quantity ) {
         ...
         iwillio_assert( fromacnt.balance >= quantity, "overdrawn balance" );
   ...
   private:
      ...
      iwillio::multi_index<N(accounts), account> _accounts;
      ...
      void add_balance( account_name payer, account_name to, uint64_t q ) {
         ...
         if( toitr == _accounts.end() ) {
           ...
           _accounts.modify( toitr, 0, [&]( auto& a ) {
              a.balance += q;
              iwillio_assert( a.balance >= q, "overflow detected" );
           });
         }
      }
};

IWILLIO_ABI( simpletoken, (transfer)(issue) )
