...
namespace snapshot_test {
   ...
   struct main_record {
      ...
      IWILLLIB_SERIALIZE( main_record, (id)(index_f64)(index_f128)(index_i64)(index_i128)(index_i256) )
   };
   ...
   struct increment {
      ...
      IWILLLIB_SERIALIZE(increment, (value))
   };

   using multi_index_type = iwillio::multi_index<N(data), main_record,
...
namespace multi_index_test {
   extern "C" {
      ...
      void apply( uint64_t self, uint64_t code, uint64_t action ) {
         require_auth(code);
         iwillio_assert(action == N(increment), "unsupported action");
