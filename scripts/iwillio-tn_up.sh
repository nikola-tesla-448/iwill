#!/bin/bash
#
# iwillio-tn_up is a helper script used to start a node that was previously stopped.
# It is not intended to be run stand-alone; it is a companion to the
# iwillio-tn_bounce.sh and iwillio-tn_roll.sh scripts.

connected="0"

rundir=programs/nodiwill
prog=nodiwill
...
if [ ! -e $rundir/$prog ]; then
    echo unable to locate binary for nodiwill
    exit -1
fi
...
datadir=var/lib/node_$IWILLIO_NODE
...
relaunch() {
    echo "$rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/iwillio/node_$IWILLIO_NODE > $datadir/stdout.txt  2>> $datadir/$log "
    nohup $rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/iwillio/node_$IWILLIO_NODE > $datadir/stdout.txt  2>> $datadir/$log &
    pid=$!
    echo pid = $pid
...
if [ -z "$IWILLIO_LEVEL" ]; then
    echo starting with no modifiers
    relaunch
    if [ "$connected" -eq 0 ]; then
        IWILLIO_LEVEL=replay
    else
        exit 0
    fi
fi

if [ "$IWILLIO_LEVEL" == replay ]; then
    echo starting with replay
    relaunch --hard-replay-blockchain
    if [  "$connected" -eq 0 ]; then
        IWILLIO_LEVEL=resync
    else
        exit 0
    fi
fi
if [ "$IWILLIO_LEVEL" == resync ]; then
