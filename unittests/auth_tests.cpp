...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   // Link authority for iwillio reqauth action with alice's spending key
   chain.link_authority("alice", "iwillio", "spending",  "reqauth");
   ...
   BOOST_CHECK_THROW( chain.link_authority("alice", "iwillio", "spending",  "reqauth"), action_validate_exception);

   // Unlink alice with iwillio reqauth
   chain.unlink_authority("alice", "iwillio", "reqauth");
   ...
   // Link authority for any iwillio action with alice's scud key
   chain.link_authority("alice", "iwillio", "scud");
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority("alice", "iwillio", "first",  "reqauth");
...
try {
   ...
   // Creating account with iwillio. prefix with privileged account
   chain.create_account("iwillio.test1");

   // Creating account with iwillio. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account("iwillio.test2", "joe"), action_validate_exception,
                         fc_exception_message_is("only privileged accounts can have names that start with 'iwillio.'"));
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( "alice", "iwillio", "iwillio.any", "reqauth" );
   chain.link_authority( "bob", "iwillio", "iwillio.any", "reqauth" );

   /// this should succeed because iwillio::reqauth is linked to any permission
   chain.push_reqauth("alice", { permission_level{N(alice), "spending"} }, { spending_priv_key });
...
try {
   ...
   using resource_usage_object = iwillio::chain::resource_limits::resource_usage_object;
   using by_owner = iwillio::chain::resource_limits::by_owner;
...
BOOST_AUTO_TEST_CASE( linkauth_special ) { try {
   ...
   auto validate_disallow = [&] (const char *type) {
      BOOST_REQUIRE_EXCEPTION(
         ...
         fc_exception_message_is(std::string("Cannot link iwillio::") + std::string(type) + std::string(" to a minimum permission"))
      );
   };
