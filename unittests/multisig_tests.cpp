...
#include <iwillio.msig/iwillio.msig.wast.hpp>
#include <iwillio.msig/iwillio.msig.abi.hpp>
...
class iwillio_msig_tester : public tester {
public:

   iwillio_msig_tester() {
      create_accounts( { N(iwillio.msig), N(iwillio.stake), N(iwillio.ram), N(iwillio.ramfee), N(alice), N(bob), N(carol) } );
      produce_block();

      auto trace = base_tester::push_action(config::system_account_name, N(setpriv),
                                            config::system_account_name,  mutable_variant_object()
                                            ("account", "iwillio.msig")
                                            ("is_priv", 1)
      );

      set_code( N(iwillio.msig), iwillio_msig_wast );
      set_abi( N(iwillio.msig), iwillio_msig_abi );

      produce_blocks();
      const auto& accnt = control->db().get<account_object,by_name>( N(iwillio.msig) );
      abi_def abi;
   ...
   void issue( name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(iwillio.token), N(issue), manager, mutable_variant_object()
                                ("to",      to )
                                ("quantity", amount )
                                ("memo", "")
                                );
   ...
   void transfer( name from, name to, const string& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(iwillio.token), N(transfer), manager, mutable_variant_object()
                                ("from",    from)
                                ("to",      to )
   ...
   asset get_balance( const account_name& act ) {
      ...
      const auto* tbl = db.find<table_id_object, by_code_scope_table>(boost::make_tuple(N(iwillio.token), act, N(accounts)));
      share_type result = 0;
   ...
   transaction_trace_ptr push_action( const account_name& signer, const action_name& name, const variant_object& data, bool auth = true ) {
      ...
      auto trace = base_tester::push_action( N(iwillio.msig), name, accounts, data );
      produce_block();
...
transaction iwillio_msig_tester::reqauth( account_name from, const vector<permission_level>& auths, const fc::microseconds& max_serialization_time ) {
   fc::variants v;
...
BOOST_AUTO_TEST_SUITE(iwillio_msig_tests)

BOOST_FIXTURE_TEST_CASE( propose_approve_execute, iwillio_msig_tester ) try {
   ...
   BOOST_REQUIRE_EXCEPTION( push_action( N(alice), N(exec), mvo()
                            ...
                            iwillio_assert_message_is("transaction authorization failed")
   );
...
BOOST_FIXTURE_TEST_CASE( propose_approve_unapprove, iwillio_msig_tester ) try {
   auto trx = reqauth("alice", {permission_level{N(alice), config::active_name}}, abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( propose_approve_by_two, iwillio_msig_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( propose_with_wrong_requested_auth, iwillio_msig_tester ) try {
   auto trx = reqauth("alice", vector<permission_level>{ { N(alice), config::active_name },  { N(bob), config::active_name } }, abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( big_transaction, iwillio_msig_tester ) try {
   vector<permission_level> perm = { { N(alice), config::active_name }, { N(bob), config::active_name } };
   auto wasm = wast_to_wasm( iwillio_token_wast );
...
BOOST_FIXTURE_TEST_CASE( update_system_contract_all_approve, iwillio_msig_tester ) try {

   // required to set up the link between (iwillio active) and (iwillio.prods active)
   //
   //                  iwillio active
   //                       |
   //             iwillio.prods active (2/3 threshold)
   ...
   set_authority(config::system_account_name, "active", authority(1,
      vector<key_weight>{{get_private_key("iwillio", "active").get_public_key(), 1}},
   ...
   create_accounts( { N(iwillio.token) } );
   set_code( N(iwillio.token), iwillio_token_wast );
   set_abi( N(iwillio.token), iwillio_token_abi );

   create_currency( N(iwillio.token), config::system_account_name, core_from_string("10000000000.0000") );
   issue(config::system_account_name, core_from_string("1000000000.0000"));
   BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"),
                        get_balance("iwillio") + get_balance("iwillio.ramfee") + get_balance("iwillio.stake") + get_balance("iwillio.ram") );

   set_code( config::system_account_name, iwillio_system_wast );
   set_abi( config::system_account_name, iwillio_system_abi );
   ...
   // execute by alice to replace the iwillio system contract
   transaction_trace_ptr trace;
   control->applied_transaction.connect([&]( const transaction_trace_ptr& t) { if (t->scheduled) { trace = t; } } );
   ...
   BOOST_REQUIRE_EXCEPTION( create_account_with_resources( N(alice1111112), config::system_account_name, core_from_string("1.0000"), false ),
                            iwillio_assert_message_exception, iwillio_assert_message_is("Unknown Test")

   );
} FC_LOG_AND_RETHROW()

BOOST_FIXTURE_TEST_CASE( update_system_contract_major_approve, iwillio_msig_tester ) try {

   // set up the link between (iwillio active) and (iwillio.prods active)
   set_authority(config::system_account_name, "active", authority(1,
      vector<key_weight>{{get_private_key("iwillio", "active").get_public_key(), 1}},
      vector<permission_level_weight>{{{config::producers_account_name, config::active_name}, 1}}), "owner",
      { { config::system_account_name, "active" } }, { get_private_key( config::system_account_name, "active" ) });
   ...
   create_currency( N(iwillio.token), config::system_account_name, core_from_string("10000000000.0000") );
   issue(config::system_account_name, core_from_string("1000000000.0000"));
   BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"), get_balance( "iwillio" ) );
   ...
   BOOST_REQUIRE_EXCEPTION(
      ...
      iwillio_assert_message_exception, iwillio_assert_message_is("transaction authorization failed")
   );
