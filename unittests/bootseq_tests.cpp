...
public:
   ...
   fc::variant get_global_state() {
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance(N(iwillio.token), symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        // Create iwillio.msig and iwillio.token
        create_accounts({N(iwillio.msig), N(iwillio.token), N(iwillio.ram), N(iwillio.ramfee), N(iwillio.stake), N(iwillio.vpay), N(iwillio.bpay), N(iwillio.saving) });

        // Set code for the following accounts:
        //  - iwillio (code: iwillio.bios) (already set by tester constructor)
        //  - iwillio.msig (code: iwillio.msig)
        //  - iwillio.token (code: iwillio.token)
        set_code_abi(N(iwillio.msig), iwillio_msig_wast, iwillio_msig_abi);//, &iwillio_active_pk);
        set_code_abi(N(iwillio.token), iwillio_token_wast, iwillio_token_abi); //, &iwillio_active_pk);

        // Set privileged for iwillio.msig and iwillio.token
        set_privileged(N(iwillio.msig));
        set_privileged(N(iwillio.token));

        // Verify iwillio.msig and iwillio.token is privileged
        const auto& iwillio_msig_acc = get<account_object, by_name>(N(iwillio.msig));
        BOOST_TEST(iwillio_msig_acc.privileged == true);
        const auto& iwillio_token_acc = get<account_object, by_name>(N(iwillio.token));
        BOOST_TEST(iwillio_token_acc.privileged == true);


        // Create SYS tokens in iwillio.token, set its manager as iwillio
        ...
        create_currency(N(iwillio.token), config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion SYS tokens to iwillio.system
        issue(N(iwillio.token), config::system_account_name, config::system_account_name, initial_supply);
        ...
        // Set iwillio.system to iwillio
        set_code_abi(config::system_account_name, iwillio_system_wast, iwillio_system_abi);
        ...
        for( const auto& a : test_genesis ) {
           ...
           r = delegate_bandwidth(N(iwillio.stake), a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == "iwillio");
        ...
        BOOST_REQUIRE_THROW(claim_rewards(N(runnerup1)), iwillio_assert_message_exception);
        ...
        BOOST_REQUIRE_THROW(undelegate_bandwidth(N(b1), N(b1), core_from_string("49999500.0000"), core_from_string("49999500.0000")), iwillio_assert_message_exception);
