...
#include <iwillio/testing/tester_network.hpp>
...
asset get_currency_balance(const TESTER& chain, account_name account) {
   return chain.get_currency_balance(N(iwillio.token), symbol(SY(4,CUR)), account);
}

const std::string iwillio_token = name(N(iwillio.token)).to_string();
...
BOOST_AUTO_TEST_CASE( link_delay_direct_test ) { try {
   ...
   chain.create_account(N(iwillio.token));
   chain.produce_blocks(10);

   chain.set_code(N(iwillio.token), iwillio_token_wast);
   chain.set_abi(N(iwillio.token), iwillio_token_abi);
   ...
   chain.push_action(config::system_account_name, linkauth::get_name(), tester_account, fc::mutable_variant_object()
           ...
           ("requirement", "first"));
   chain.produce_blocks();
   chain.push_action(N(iwillio.token), N(create), N(iwillio.token), mutable_variant_object()
           ("issuer", iwillio_token)
           ("maximum_supply", "9000000.0000 CUR")
   );


   chain.push_action(N(iwillio.token), name("issue"), N(iwillio.token), fc::mutable_variant_object()
           ("to",       iwillio_token)
           ("quantity", "1000000.0000 CUR")
           ("memo", "for stuff")
   );

   auto trace = chain.push_action(N(iwillio.token), name("transfer"), N(iwillio.token), fc::mutable_variant_object()
       ("from", iwillio_token)
       ("to", "tester")
   ...
   auto liquid_balance = get_currency_balance(chain, N(iwillio.token));
   ...
   trace = chain.push_action(N(iwillio.token), name("transfer"), N(tester), fc::mutable_variant_object()
       ("from", "tester")
       ("to", "tester2")
       ("quantity", "1.0000 CUR")
       ("memo", "hi" )
   ...
   liquid_balance = get_currency_balance(chain, N(iwillio.token));
   BOOST_REQUIRE_EQUAL(asset::from_string("999900.0000 CUR"), liquid_balance);
   liquid_balance = get_currency_balance(chain, N(tester));
...
BOOST_AUTO_TEST_CASE(delete_auth_test) { try {
   ...
   chain.produce_blocks();
   chain.push_action(N(iwillio.token), N(create), N(iwillio.token), mutable_variant_object()
           ("issuer", "iwillio.token" )
           ("maximum_supply", "9000000.0000 CUR" )
   );

   // issue to account "iwillio.token"
   chain.push_action(N(iwillio.token), name("issue"), N(iwillio.token), fc::mutable_variant_object()
           ("to",       "iwillio.token")
           ("quantity", "1000000.0000 CUR")
           ("memo", "for stuff")
   ...
   // transfer from iwillio.token to tester
   trace = chain.push_action(N(iwillio.token), name("transfer"), N(iwillio.token), fc::mutable_variant_object()
       ("from", "iwillio.token")
       ("to", "tester")
       ("quantity", "100.0000 CUR")
       ("memo", "hi" )
...
BOOST_AUTO_TEST_CASE( mindelay_test ) { try {
   ...
   const auto& acnt = chain.control->db().get<account_object,by_name>(N(iwillio.token));
   const auto abi = acnt.get_abi();
   chain::abi_serializer abis(abi, chain.abi_serializer_max_time);
   const auto a = chain.control->db().get<account_object,by_name>(N(iwillio.token)).get_abi();

   string action_type_name = abis.get_action_type(name("transfer"));

   action act;
   act.account = N(iwillio.token);
...
BOOST_AUTO_TEST_CASE( canceldelay_test2 ) { try {
...

   {
      ...
      trace = chain.push_action(N(iwillio.token), name("transfer"), vector<permission_level>{{N(tester), N(first)}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "1.0000 CUR")
          ("memo", "hi" ),
   ...
   {
      // this transaction will be delayed 10 blocks
      trace = chain.push_action(N(iwillio.token), name("transfer"), vector<permission_level>{{N(tester), N(second)}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "5.0000 CUR")
          ("memo", "hi" ),
   ...
   {
      // this transaction will be delayed 10 blocks
      trace = chain.push_action(N(iwillio.token), name("transfer"), vector<permission_level>{{N(tester), config::owner_name}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "10.0000 CUR")
          ("memo", "hi" ),
