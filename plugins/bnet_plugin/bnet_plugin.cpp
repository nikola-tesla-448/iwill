...
#include <iwillio/bnet_plugin/bnet_plugin.hpp>
...
using iwillio::public_key_type;
using iwillio::chain_id_type;
using iwillio::block_id_type;
using iwillio::block_timestamp_type;
using std::string;
using iwillio::sha256;
using iwillio::signed_block_ptr;
using iwillio::packed_transaction_ptr;
using std::vector;
...
namespace iwillio {
       ...
       //    shared_ptr< vector<char> >  block_msg; ///< packed bnet_message for this block
        ...
        void on( const signed_block_ptr& b ) {
           ...
           if (!b) {
              ...
              IWILL_THROW(block_validate_exception, "bad block" );
           }
        ...
        void on( const packed_transaction_ptr& p ) {
           ...
           if (!p) {
              ...
              IWILL_THROW(transaction_exception, "bad transaction");
           }
  ...
  class listener : public std::enable_shared_from_this<listener> {
     ...
     public:
        ...
        void run() {
           IWILL_ASSERT( _acceptor.is_open(), plugin_exception, "unable top open listen socket" );
           do_accept();
        }
   ...
   void bnet_plugin::plugin_shutdown() {
      ...
      my->for_each_session([](auto ses){
         IWILL_ASSERT( false, plugin_exception, "session ${ses} still active", ("ses", ses->_session_num) );
      });
