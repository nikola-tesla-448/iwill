/**
 *  @file
 *  @copyright defined in iwillio/LICENSE.txt
 */
...
{
   try {
      app().set_version(iwillio::nodiwill::config::version);
      app().register_plugin<history_plugin>();

      auto root = fc::app_path();
      app().set_default_data_dir(root / "iwillio/nodiwill/data" );
      app().set_default_config_dir(root / "iwillio/nodiwill/config" );
      ...
      initialize_logging();
      ilog("nodiwill version ${ver}", ("ver", app().version_string()));
      ilog("iwillio root is ${root}", ("root", root.string()));
      app().startup();
      app().exec();
