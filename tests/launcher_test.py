...
try:
    ...
    if not dontLaunch:
        ...
        killIwillInstances=False

        print("Stand up walletd")
        if walletMgr.launch() is False:
            cmdError("%s" % (WalletdName))
            errorExit("Failed to stand up iwill walletd.")
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.iwillioAccount,cluster.defproduceraAccount])
    ...
    Print("Create new account %s via %s" % (testeraAccount.name, cluster.iwillioAccount.name))
    transId=node.createInitializeAccount(testeraAccount, cluster.iwillioAccount, stakedDeposit=0, waitForTransBlock=False, exitOnError=True)

    Print("Create new account %s via %s" % (currencyAccount.name, cluster.iwillioAccount.name))
    transId=node.createInitializeAccount(currencyAccount, cluster.iwillioAccount, buyRAM=1000000, stakedDeposit=5000, exitOnError=True)

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.iwillioAccount.name))
    transId=node.createInitializeAccount(exchangeAccount, cluster.iwillioAccount, buyRAM=1000000, waitForTransBlock=True, exitOnError=True)
    ...
    actualAmount=node.getAccountIwillBalanceStr(testeraAccount.name)
    ...
    Print("Verify transfer, Expected: %s" % (expectedAmount))
    actualAmount=node.getAccountIwillBalanceStr(currencyAccount.name)
    if expectedAmount != actualAmount:
        cmdError("FAILURE - transfer failed")
        errorExit("Transfer verification failed. Excepted %s, actual: %s" % (expectedAmount, actualAmount))
