...
killAll()
{
  programs/iwillio-launcher/iwillio-launcher -k 15
}

cleanup()
{
    rm -rf etc/iwillio/node_*
    rm -rf var/lib/node_*
}
...
getHeadBlockNum()
{
  INFO="$(programs/cliwill/cliwill get info)"
  verifyErrorCode "cliwill get info"
...
# stand up nodiwill cluster
launcherOpts="-p $pnodes -n $total_nodes -s $topo -d $delay"
echo Launcher options: --nodiwill \"--plugin iwillio::wallet_api_plugin\" $launcherOpts
programs/iwillio-launcher/iwillio-launcher --nodiwill "--plugin iwillio::wallet_api_plugin" $launcherOpts
sleep 7
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(programs/cliwill/cliwill --port $port2 get block 1)"
    verifyErrorCode "cliwill get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(programs/cliwill/cliwill create key)"
verifyErrorCode "cliwill create key"
...
PASSWORD_INITA="$(programs/cliwill/cliwill wallet create --name inita)"
verifyErrorCode "cliwill wallet create"
# strip out password from output
PASSWORD_INITA="$(echo "$PASSWORD_INITA" | awk '/PW/ {print $1}')"
# remove leading/trailing quotes
PASSWORD_INITA=${PASSWORD_INITA#\"}
PASSWORD_INITA=${PASSWORD_INITA%\"}
programs/cliwill/cliwill wallet import --name inita --private-key $INITA_PRV_KEY
verifyErrorCode "cliwill wallet import"
programs/cliwill/cliwill wallet import --name inita --private-key $PRV_KEY1
verifyErrorCode "cliwill wallet import"
programs/cliwill/cliwill wallet import --name inita --private-key $PRV_KEY2
verifyErrorCode "cliwill wallet import"
programs/cliwill/cliwill wallet import --name inita --private-key $PRV_KEY3
verifyErrorCode "cliwill wallet import"
...
ACCOUNT_INFO="$(programs/cliwill/cliwill create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "cliwill create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(programs/cliwill/cliwill get account testera)"
verifyErrorCode "cliwill get account"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(programs/cliwill/cliwill transfer inita testera 975321 "test transfer")"
    verifyErrorCode "cliwill transfer"
   ...
   TRANS_INFO="$(programs/cliwill/cliwill --port $port2 get transaction $TRANS_ID)"
   verifyErrorCode "cliwill get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done
