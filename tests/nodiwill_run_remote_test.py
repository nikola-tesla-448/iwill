...
###############################################################
# nodiwill_run_remote_test
#  Tests remote capability of the nodiwill_run_test. Test will setup cluster and pass nodes info to nodiwill_run_test. E.g.
#  nodiwill_run_remote_test.py -v --clean-run --dump-error-detail
###############################################################
...
pnodes=1
total_nodes=pnodes
actualTest="tests/nodiwill_run_test.py"
...
try:
    ...
    Print("Starting up %s test: %s" % ("nodiwill", actualTest))
    Print("cmd: %s\n" % (cmd))
    ...
    TestHelper.shutdown(cluster, None, testSuccessful, killIwillInstances, False, False, killAll, dumpErrorDetails)
