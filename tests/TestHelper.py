...
class TestHelper(object):
    ...
    def parse_args(includeArgs, applicationSpecificArgs=AppArgs()):
        ...
        if "--kill-count" in includeArgs:
            parser.add_argument("--kill-count", type=int, help="nodiwill instances to kill", default=-1)
        ...
        if "--host" in includeArgs:
            parser.add_argument("-h", "--host", type=str, help="%s host name" % (Utils.IwillServerName),
                                     default=TestHelper.LOCAL_HOST)
        if "--port" in includeArgs:
            parser.add_argument("-p", "--port", type=int, help="%s host port" % Utils.IwillServerName,
                                     default=TestHelper.DEFAULT_PORT)
        if "--wallet-host" in includeArgs:
            parser.add_argument("--wallet-host", type=str, help="%s host" % Utils.IwillWalletName,
                                     default=TestHelper.LOCAL_HOST)
        if "--wallet-port" in includeArgs:
            parser.add_argument("--wallet-port", type=int, help="%s port" % Utils.IwillWalletName,
        ...
        if "--clean-run" in includeArgs:
            parser.add_argument("--clean-run", help="Kill all nodiwill and kliwill instances", action='store_true')
        if "--sanity-test" in includeArgs:
            parser.add_argument("--sanity-test", help="Validates nodiwill and kliwill are in path and can be started up.", action='store_true')
    ...
    def printSystemInfo(prefix):
        ...
        Utils.Print("IWILL Client version: %s" % (clientVersion))
        Utils.Print("Processor: %s" % (platform.processor()))
        Utils.Print("OS name: %s" % (platform.platform()))
    
    @staticmethod
    # pylint: disable=too-many-arguments
    def shutdown(cluster, walletMgr, testSuccessful=True, killIwillInstances=True, killWallet=True, keepLogs=False, cleanRun=True, dumpErrorDetails=False):
        ...
        assert(isinstance(killIwillInstances, bool))
