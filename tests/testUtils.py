...
class Utils:
    ...
    IwillClientPath="programs/cliwill/cliwill"
    MiscIwillClientArgs="--no-auto-kiwilld"

    IwillWalletName="kiwilld"
    IwillWalletPath="programs/kiwilld/"+ IwillWalletName

    IwillServerName="nodiwill"
    IwillServerPath="programs/nodiwill/"+ IwillServerName

    IwillLauncherPath="programs/iwillio-launcher/iwillio-launcher"
    MongoPath="mongo"
    ShuttingDown=False
    CheckOutputDeque=deque(maxlen=10)

    IwillBlockLogPath="programs/iwillio-blocklog/iwillio-blocklog"
    ...
    def getBlockLog(blockLogLocation, silentErrors=False, exitOnError=False):
        ...
        cmd="%s --blocks-dir %s --as-json-array" % (Utils.IwillBlockLogPath, blockLogLocation)
