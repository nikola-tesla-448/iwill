...
# --kill-count <nodiwill instances to kill>
...
try:
    ...
    Print("Stand up IWILL wallet kiwilld")
    accountsCount=total_nodes
    walletName="MyWallet"
    Print("Creating wallet %s if one doesn't already exist." % walletName)
    wallet=walletMgr.create(walletName, [cluster.iwillioAccount,cluster.defproduceraAccount,cluster.defproducerbAccount])
    ...
    iwillioAccount=cluster.iwillioAccount
    ...
    if not cluster.createAccounts(iwillioAccount):
    ...
    if cluster.killSomeIwillInstances(killCount, killSignal) is False:
        errorExit("Failed to kill Iwill instances")
    Print("nodiwill instances killed.")
    ...
    if cluster.relaunchIwillInstances() is False:
        errorExit("Failed to relaunch Iwill instances")
    Print("nodiwill instances relaunched.")
    ...
    TestHelper.shutdown(cluster, walletMgr, testSuccessful, killIwillInstances, killIwillInstances, keepLogs, killAll, dumpErrorDetails)
