...
my $iwill_home = defined $ENV{IWILL_HOME} ? $ENV{IWILL_HOME} : getcwd;
my $iwilld = $iwill_home . "/programs/iwilld/iwilld";
my $iwillc = $iwill_home . "/programs/iwillc/iwillc";

my $nodes = defined $ENV{IWILL_TEST_RING} ? $ENV{IWILL_TEST_RING} : "1";
my $pnodes = defined $ENV{IWILL_TEST_PRODUCERS} ? $ENV{IWILL_TEST_PRODUCERS} : "1";

my $prods = 21;
my $genesis = "$iwill_home/genesis.json";
...
if (!GetOptions("nodes=i" => \$nodes,
    ...
    print "where:\n";
    print "--nodes=n (default = 1) sets the number of iwilld instances to launch\n";
...
sub write_config {
    ...
    if (defined $producer) {
        ...
        print $cfg "private-key = [\"IWILL6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV\",\"5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3\"]\n";

        print $cfg "plugin = iwillio::producer_plugin\n";
        print $cfg "plugin = iwillio::chain_api_plugin\n";
...
sub perform_work {
    ...
    if ($mode == 0) {
        ...
        my $counter = 0;
        while (time () < $stoptime) {
            `$iwillc transfer iwill inita 10 >> iwillc.out 2>> iwillc.err`;
            $counter++;
            if ($counter % 1000 == 0) {
