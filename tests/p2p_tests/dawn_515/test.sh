...
read -d '' genesis << EOF
{
  ...
  "initial_key": "IWILL6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV",
...
programs/iwillio-launcher/iwillio-launcher -p $pnodes -n $total_nodes --nogen -d $delay
...
b5idbios=`./programs/cliwill/cliwill -u http://localhost:8888 get block 5 | grep "^ *\"id\""`
b5id00=`./programs/cliwill/cliwill -u http://localhost:8889 get block 5 | grep "^ *\"id\""`
b5id01=`./programs/cliwill/cliwill -u http://localhost:8890 get block 5 | grep "^ *\"id\""`
