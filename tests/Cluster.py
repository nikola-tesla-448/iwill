...
class Cluster(object):
    ...
    __BiosPort=8788
    __LauncherCmdArr=[]
    __bootlog="iwillio-ignition-wd/bootlog.txt"
    __configDir="etc/iwillio/"
    __dataDir="var/lib/"
    __fileDivider="================================================================="

    # pylint: disable=too-many-arguments
    # walletd [True|False] Is kiwilld running. If not load the wallet plugin
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899, enableMongo=False
                 , mongoHost="localhost", mongoPort=27017, mongoDb="IWILLtest", defproduceraPrvtKey=None, defproducerbPrvtKey=None, staging=False):
        """Cluster container.
        walletd [True|False] Is wallet kiwilld running. If not load the wallet plugin
        localCluster [True|False] Is cluster local to host.
        host: iwill server host
        port: iwill server port
        walletHost: iwill wallet host
        walletPort: wos wallet port
        enableMongo: Include mongoDb support, configures iwill mongo plugin
        ...
        self.iwillioAccount=self.defProducerAccounts["iwillio"]= Account("iwillio")
    ...
    def launch(self, pnodes=1, totalNodes=1, prodCount=1, topo="mesh", p2pPlugin="net", delay=1, onlyBios=False, dontBootstrap=False,
               totalProducers=None, extraNodiwillArgs=None, useBiosBootFile=True, specificExtraNodiwillArgs=None):
        """Launch cluster.
        ...
        extraNodiwillArgs: string of arguments to pass through to each nodoes instance (via --nodiwill flag on launcher)
        ...
        specificExtraNodiwillArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-nodiwill flags on launcher), example: { "5" : "--plugin iwillio::test_control_api_plugin" }
        """
        assert(isinstance(topo, str))

        if not self.localCluster:
            Utils.Print("WARNING: Cluster not local, not launching %s." % (Utils.IwillServerName))
            return True
        ...
        while not Utils.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on nodiwill default port. wait...")
            if tries == 0:
                return False
            tries = tries - 1
            time.sleep(2)
        ...
        cmd="%s -p %s -n %s -d %s -i %s -f --p2p-plugin %s %s" % (
            Utils.IwillLauncherPath, pnodes, totalNodes, delay, datetime.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3],
            p2pPlugin, producerFlag)
        cmdArr=cmd.split()
        if self.staging:
            cmdArr.append("--nogen")

        nodiwillArgs="--max-transaction-time -1 --abi-serializer-max-time-ms 990000 --filter-on * --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
            nodiwillArgs += " --plugin iwillio::wallet_api_plugin"
        if self.enableMongo:
            nodiwillArgs += " --plugin iwillio::mongo_db_plugin --mongodb-wipe --delete-all-blocks --mongodb-uri %s" % self.mongoUri
        if extraNodiwillArgs is not None:
            assert(isinstance(extraNodiwillArgs, str))
            nodiwillArgs += extraNodiwillArgs
        if Utils.Debug:
            nodiwillArgs += " --contracts-console"

        if nodiwillArgs:
            cmdArr.append("--nodiwill")
            cmdArr.append(nodiwillArgs)

        if specificExtraNodiwillArgs is not None:
            assert(isinstance(specificExtraNodiwillArgs, dict))
            for nodeNum,arg in specificExtraNodiwillArgs.items():
                assert(isinstance(nodeNum, (str,int)))
                assert(isinstance(arg, str))
                cmdArr.append("--specific-num")
                cmdArr.append(str(nodeNum))
                cmdArr.append("--specific-nodiwill")
                cmdArr.append(arg)
        ...
        if nodes is None or totalNodes != len(nodes):
            Utils.Print("ERROR: Unable to validate %s instances, expected: %d, actual: %d" %
                          (Utils.IwillServerName, totalNodes, len(nodes)))
            return False
        ...
        self.iwillioAccount=self.defProducerAccounts["iwillio"]
    ...
    def getClientVersion(verbose=False):
        ...
        try:
            cmd="%s version client" % (Utils.IwillClientPath)
    ...
    def createAccountKeys(count):
        ...
        for _ in range(0, count):
            try:
                cmd="%s create key --to-console" % (Utils.IwillClientPath)
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        fromm=source
        to=accounts[0]
        Utils.Print("Transfer %s units from account %s to %s on iwill server port %d" % (
        ...
        for i in range(0, count):
            ...
            for _ in range(0, count):
                #Utils.Print("nextIwillIdx: %d, n: %d" % (nextIwillIdx, n))
                nextIwillIdx=(nextIwillIdx + 1)%count
                if not self.nodes[nextIwillIdx].killed:
                    #Utils.Print("nextIwillIdx: %d" % (nextIwillIdx))
            ...
            #Utils.Print("nextIwillIdx: %d, count: %d" % (nextIwillIdx, count))
            node=self.nodes[nextIwillIdx]
    ...
    def validateSpreadFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        for node in self.nodes:
            ...
            if Utils.Debug: Utils.Print("Validate funds on %s server port %d." %
                                        (Utils.IwillServerName, node.port))

            if node.validateFunds(initialBalances, transferAmount, source, accounts) is False:
                Utils.Print("ERROR: Failed to validate funds on iwill node port: %d" % (node.port))
                return False
    ...
    def spreadFundsAndValidate(self, transferAmount=1):
        ...
        initialBalances=self.nodes[0].getIwillBalances([self.defproduceraAccount] + self.accounts)
        assert(initialBalances)
        assert(isinstance(initialBalances, dict))
    ...
    def validateAccounts(self, accounts, testSysAccounts=True):
        ...
        myAccounts = []
        if testSysAccounts:
            myAccounts += [self.iwillioAccount, self.defproduceraAccount, self.defproducerbAccount]
        if accounts:
            assert(isinstance(accounts, list))
            myAccounts += accounts
    ...
    def parseClusterKeys(totalNodes):
        ...
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse iwillio private keys from cluster config files.")
            return None
    ...
    def bios_bootstrap(totalNodes, biosHost, biosPort, walletMgr, silent=False):
        """Bootstrap cluster using the bios_boot.sh script generated by iwillio-launcher."""
        ...
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
            if not silent: Utils.Print("Launcher failed to shut down iwill cluster.")
            return None
        ...
        iwillioName="iwillio"
        iwillioKeys=producerKeys[iwillioName]
        iwillioAccount=Account(iwillioName)
        iwillioAccount.ownerPrivateKey=iwillioKeys["private"]
        iwillioAccount.ownerPublicKey=iwillioKeys["public"]
        iwillioAccount.activePrivateKey=iwillioKeys["private"]
        iwillioAccount.activePublicKey=iwillioKeys["public"]
        producerKeys.pop(iwillioName)

        if not walletMgr.importKey(iwillioAccount, ignWallet):
            Utils.Print("ERROR: Failed to import %s account keys into ignition wallet." % (iwillioName))
            return None

        initialFunds="1000000.0000 {0}".format(CORE_SYMBOL)
        Utils.Print("Transfer initial fund %s to individual accounts." % (initialFunds))
        trans=None
        contract="iwillio.token"
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"iwillio\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (name, initialFunds, "init transfer")
            opts="--permission iwillio@active"
            if name != "iwillio":
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from iwillio.token to %s." % (name))
                    return None
    ...
    def bootstrap(totalNodes, prodCount, totalProducers, biosHost, biosPort, walletMgr, onlyBios=False):
        ...
        contract="iwillio.bios"
        contractDir="contracts/%s" % (contract)
        wasmFile="%s.wasm" % (contract)
        abiFile="%s.abi" % (contract)
        Utils.Print("Publish %s contract" % (contract))
        trans=biosNode.publishContract(iwillioAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
        ...
        for name, keys in producerKeys.items():
            ...
            trans=biosNode.createAccount(initx, iwillioAccount, 0)
            if trans is None:
        ...
        if not onlyBios:
            if prodCount == -1:
                ...
                with open(setProdsFile, "r") as f:
                    ...
                    opts="--permission iwillio@active"
                    myTrans=biosNode.pushMessage("iwillio", "setprods", setProdsStr, opts)
                ...
                trans=biosNode.pushMessage("iwillio", "setprods", setProdsStr, opts)
                if trans is None or not trans[0]:
            ...
            # wait for block production handover (essentially a block produced by anyone but iwillio).
            lam = lambda: biosNode.getInfo(exitOnError=True)["head_block_producer"] != "iwillio"
            ret=Utils.waitForBool(lam)
            if not ret:
                Utils.Print("ERROR: Block production handover failed.")
                return None

        iwillioTokenAccount=copy.deepcopy(iwillioAccount)
        iwillioTokenAccount.name="iwillio.token"
        trans=biosNode.createAccount(iwillioTokenAccount, iwillioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (iwillioTokenAccount.name))
            return None

        iwillioRamAccount=copy.deepcopy(iwillioAccount)
        iwillioRamAccount.name="iwillio.ram"
        trans=biosNode.createAccount(iwillioRamAccount, iwillioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (iwillioRamAccount.name))
            return None

        iwillioRamfeeAccount=copy.deepcopy(iwillioAccount)
        iwillioRamfeeAccount.name="iwillio.ramfee"
        trans=biosNode.createAccount(iwillioRamfeeAccount, iwillioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (iwillioRamfeeAccount.name))
            return None

        iwillioStakeAccount=copy.deepcopy(iwillioAccount)
        iwillioStakeAccount.name="iwillio.stake"
        trans=biosNode.createAccount(iwillioStakeAccount, iwillioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (iwillioStakeAccount.name))
            return None
        ...
        abiFile="%s.abi" % (contract)
        Utils.Print("Publish %s contract" % (contract))
        trans=biosNode.publishContract(iwillioTokenAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
            Utils.Print("ERROR: Failed to publish contract %s." % (contract))
            return None

        # Create currency0000, followed by issue currency0000
        contract=iwillioTokenAccount.name
        Utils.Print("push create action to %s contract" % (contract))
        action="create"
        data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\",\"can_freeze\":\"0\",\"can_recall\":\"0\",\"can_whitelist\":\"0\"}" % (iwillioTokenAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (contract)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push create action to iwillio contract.")
            return None
        ...
        action="issue"
        data="{\"to\":\"%s\",\"quantity\":\"1000000000.0000 %s\",\"memo\":\"initial issue\"}" % (iwillioAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (contract)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push issue action to iwillio contract.")
            return None
        ...
        Utils.Print("Verify iwillio issue, Expected: %s" % (expectedAmount))
        actualAmount=biosNode.getAccountIwillBalanceStr(iwillioAccount.name)
        if expectedAmount != actualAmount:
            Utils.Print("ERROR: Issue verification failed. Excepted %s, actual: %s" %
                        (expectedAmount, actualAmount))
            return None

        contract="iwillio.system"
        contractDir="contracts/%s" % (contract)
        wasmFile="%s.wasm" % (contract)
        abiFile="%s.abi" % (contract)
        Utils.Print("Publish %s contract" % (contract))
        ...
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"%s\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (iwillioAccount.name, name, initialFunds, "init transfer")
            opts="--permission %s@active" % (iwillioAccount.name)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to transfer funds from %s to %s." % (iwillioTokenAccount.name, name))
                return None
    ...
    @staticmethod
    def pgrepIwillServers(timeout=None):
        cmd=Utils.pgrepCmd(Utils.IwillServerName)
    ...
    @staticmethod
    def pgrepIwillServerPattern(nodeInstance):
        dataLocation=Cluster.__dataDir + Cluster.nodeExtensionToName(nodeInstance)
        return r"[\n]?(\d+) (.* --data-dir %s .*)\n" % (dataLocation)

    # Populates list of IwillInstanceInfo objects, matched to actual running instances
    def discoverLocalNodes(self, totalNodes, timeout=None):
        nodes=[]

        psOut=Cluster.pgrepIwillServers(timeout)
        if psOut is None:
        ...
        for i in range(0, totalNodes):
            pattern=Cluster.pgrepIwillServerPattern(i)
            m=re.search(pattern, psOut, re.MULTILINE)
            if m is None:
                Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.IwillServerName, pattern))
                break
    ...
    def discoverBiosNodePid(self, timeout=None):
        psOut=Cluster.pgrepIwillServers(timeout=timeout)
        pattern=Cluster.pgrepIwillServerPattern("bios")
        Utils.Print("pattern={\n%s\n}, psOut=\n%s\n" % (pattern,psOut))
        m=re.search(pattern, psOut, re.MULTILINE)
        if m is None:
            Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.IwillServerName, pattern))
    ...
    # Kills a percentange of Iwill instances starting from the tail and update iwillInstanceInfos state
    def killSomeIwillInstances(self, killCount, killSignalStr=Utils.SigKillTag):
        killSignal=signal.SIGKILL
        if killSignalStr == Utils.SigTermTag:
            killSignal=signal.SIGTERM
        Utils.Print("Kill %d %s instances with signal %s." % (killCount, Utils.IwillServerName, killSignal))
    ...
    def relaunchIwillInstances(self):
    ...
    def killall(self, silent=True, allInstances=False):
        """Kill cluster nodiwill instances. allInstances will kill all nodiwill instances running on the system."""
        cmd="%s -k 9" % (Utils.IwillLauncherPath)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
            if not silent: Utils.Print("Launcher failed to shut down iwill cluster.")

        if allInstances:
            # ocassionally the launcher cannot kill the iwill server
            cmd="pkill -9 %s" % (Utils.IwillServerName)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
                if not silent: Utils.Print("Failed to shut down iwill cluster.")
    ...
    def bounce(self, nodes, silent=True):
        """Bounces nodiwill instances as indicated by parameter nodes.
    ...
    def down(self, nodes, silent=True):
        """Brings down nodiwill instances as indicated by parameter nodes.
        nodes should take the form of a comma-separated list as accepted by the launcher --bounce command (e.g. '00' or '00,01')"""
        cmdArr = Cluster.__LauncherCmdArr.copy()
        cmdArr.append("--down")
        cmdArr.append(nodes)
