...
try:
    ...
    if not dontLaunch:
        walletAccounts.append(cluster.iwillioAccount)
    ...
    contractDir="contracts/iwillio.token"
    wasmFile="iwillio.token.wasm"
    abiFile="iwillio.token.abi"
    ...
    if not enableMongo:
        ...
        account=node.getIwillAccountFromDb(currencyAccount.name)
        ...
        if abiName != "transfer" or abiActionName != "transfer" or abiType != "transfer":
            errorExit("FAILURE - get IWILL account failed", raw=True)
    ...
    account=node.getIwillAccount(defproduceraAccount.name, exitOnError=True)
