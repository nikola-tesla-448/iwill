...
###############################################################
# nodiwill_voting_test
# --dump-error-details <Upon error print etc/iwillio/node_*/config.ini and var/lib/node_*/stderr.log to stdout>
...
killIwillInstances=not dontKill
killWallet=not dontKill

WalletdName=Utils.IwillWalletName
ClientName="cliwill"
...
try:
    ...
    maxRAMValue=1010
    extraNodiwillArgs=" %s %d %s %d " % (minRAMFlag, minRAMValue, maxRAMFlag, maxRAMValue)
    if cluster.launch(onlyBios=False, pnodes=totalNodes, totalNodes=totalNodes, totalProducers=totalNodes, extraNodiwillArgs=extraNodiwillArgs, useBiosBootFile=False) is False:
        Utils.cmdError("launcher")
        errorExit("Failed to stand up iwill cluster.")
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.iwillioAccount])
    ...
    # create accounts via iwillio as otherwise a bid is needed
    for account in accounts:
        Print("Create new account %s via %s" % (account.name, cluster.iwillioAccount.name))
        trans=nodes[0].createInitializeAccount(account, cluster.iwillioAccount, stakedDeposit=500000, waitForTransBlock=False, stakeNet=50000, stakeCPU=50000, buyRAM=50000, exitOnError=True)
        transferAmount="70000000.0000 {0}".format(CORE_SYMBOL)
        Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.iwillioAccount.name, account.name))
        nodes[0].transferFunds(cluster.iwillioAccount, account, transferAmount, "test transfer")
    ...
    Print("Create new account %s via %s" % (contractAccount.name, cluster.iwillioAccount.name))
    trans=nodes[0].createInitializeAccount(contractAccount, cluster.iwillioAccount, stakedDeposit=500000, waitForTransBlock=False, stakeNet=50000, stakeCPU=50000, buyRAM=50000, exitOnError=True)
    transferAmount="90000000.0000 {0}".format(CORE_SYMBOL)
    Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.iwillioAccount.name, contractAccount.name))
    nodes[0].transferFunds(cluster.iwillioAccount, contractAccount, transferAmount, "test transfer")
        ...
        for fromIndex in range(namedAccounts.numAccounts):
            ...
            try:
                ...
                if trans is None or not trans[0]:
                    timeOutCount+=1
                    if timeOutCount>=3:
                       Print("Failed to push create action to iwillio contract for %d consecutive times, looks like nodiwill already exited." % (timeOutCount))
                       keepProcessing=False
                       break
                    Print("Failed to push create action to iwillio contract. sleep for 60 seconds")
                    time.sleep(60)
                else:
                    timeOutCount=0
                time.sleep(1)
    ...
    TestHelper.shutdown(cluster, walletMgr, testSuccessful, killIwillInstances, killWallet, keepLogs, killAll, dumpErrorDetails)
