...
namespace iwillio { namespace chain { namespace config {
...
const static uint64_t system_account_name    = N(iwillio);
const static uint64_t null_account_name      = N(iwillio.null);
const static uint64_t producers_account_name = N(iwillio.prods);
...
const static uint64_t iwillio_auth_scope       = N(iwillio.auth);
const static uint64_t iwillio_all_scope        = N(iwillio.all);

const static uint64_t active_name = N(active);
const static uint64_t owner_name  = N(owner);
const static uint64_t iwillio_any_name = N(iwillio.any);
const static uint64_t iwillio_code_name = N(iwillio.code);
...
const static iwillio::chain::wasm_interface::vm_type default_wasm_runtime = iwillio::chain::wasm_interface::vm_type::wabt;
...
} } } // namespace iwillio::chain::config

constexpr uint64_t IWILL_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / iwillio::chain::config::percent_100;
}

template<typename Number>
Number IWILL_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + iwillio::chain::config::percent_100 - iwillio::chain::config::percent_1)  / iwillio::chain::config::percent_100;
}
