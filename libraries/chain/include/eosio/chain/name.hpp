...
#define N(X) iwillio::chain::string_to_name(#X)
...
namespace std {
   template<> struct hash<iwillio::chain::name> : private hash<uint64_t> {
      typedef iwillio::chain::name argument_type;
...
namespace fc {
  class variant;
  void to_variant(const iwillio::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, iwillio::chain::name& check);
} // fc


FC_REFLECT( iwillio::chain::name, (value) )
